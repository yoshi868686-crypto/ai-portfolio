...
ヒーローと背景の2枚は GPT Image 2.0（fal 経由）で生成した。spec は
`ユニコ/note_images/specs/portfolio-hero.json`、元 PNG は `ユニコ/note_images/` に残してある。

| 用途 | ファイル | 元 PNG |
|---|---|---|
| ヒーロー | `hero-kawaii.webp` 1400x1120 / 54KB | `portfolio_hero_2k.png` 2560x1344 |
| 背景 | `bg-kawaii.webp` 1200x1200 / 10KB | `portfolio_bg_2k.png` 2048x2048 |

**背景画像を全面に敷いていないのは、測った結果そうしたほうがよいと判断したため。**
この画像は面積の 0.27% だが **純黒 (rgb 0,0,0) のピクセルを含む**（散らばった図形の輪郭線）。
黒本文がそこに重なると比が 1:1 になって文字が消える。平均輝度では判定できない。

最初は右下に `position: fixed` で置いたが、**fixed はスクロールしても留まるので、
ページを送るたびに別の本文がその上を通る。**実測でスクロール75%の位置で6つの本文ブロック
（`.sec-note` や `h3` を含む）が重なっていた。固定配置はこの画像には使えない。

いまは `.art-ground` をフッター最下段の帯として置いている。本文が一切乗らないので安全で、
全スクロール位置・375px / 1425px の両方で重なり **0件** を実測済み。
背景画像の配置を変えるときは、必ず**スクロールさせながら**重なりを数えること。

グラデーションを潰す SVG filter（`feComponentTransfer` の discrete）も検討したが、
**実際に掛けてみたら地色が純白からピンクに転び、顔にノイズが出たので採用していない。**
計算値どおりには動かない。掛けるなら必ず1枚出して目視すること。
...
`app.js` の `filters()` は `.filters` / `[data-filterable]` / `[data-cat]` しか見ない汎用実装なので、
**Works と Log で同じコードを共有している。**新しいページに絞り込みを足すときも JS は触らない。
ボタンのラベルに書く件数は、`data-cat` の実数と一致させること（ずれると嘘になる）。

ライトボックスは `.prose figure img` と `.work-card.is-static .thumb img` だけを対象にする。
**リンクになっているカードには付けない** — クリックの本来の動作は遷移だから。

画像は**原寸で出して、収まらないぶんは枠の中でスクロールさせる**。
最初は `width: 100%` で枠に合わせていたが、実測すると拡大表示が 471px、サムネイルが 483px で
**拡大したのに小さくなっていた**。いまは 1425px 幅で 1.67倍、375px 幅で 3.57倍。
